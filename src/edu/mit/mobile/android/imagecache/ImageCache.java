...
public class ImageCache extends DiskCache<String, Bitmap> {
	...
	private DrawableMemCache<String> mMemCache = new DrawableMemCache<String>(
			DEFAULT_CACHE_SIZE);
	...
	// this is a custom Executor, as we want to have the tasks loaded in FILO
	// order. FILO works
	...
	/**
	 * Generally, it's best to use the shared image cache using
	 * {@link #getInstance(Context)}. Use this if you want to customize a cache
	 * or keep it separate.
	 * 
	 * @param context
	 * @param format
	...
	/**
	 * Set the image quality. Hint to the compressor, 0-100. 0 meaning compress
	 * for small size, 100 meaning compress for max quality. Some formats, like
	...
	/**
	 * Sets the maximum size of the memory cache. Note, this will clear the
	 * memory cache.
	 * 
	 * @param maxSize
	 ...
	 * {@link #scheduleLoadImage(int, Uri, int, int)} or indirectly by
	 * {@link #loadImage(int, Uri, int, int)}, any registered listeners will get
	 * called.
	 * </p>
	 * 
	 * <p>
	...
	/**
	 * Gets the given key as a drawable, retrieving it from memory cache if it's
	 * present.
	 * 
	 * @param key
	 *            a key generated by {@link #getKey(Uri)} or
	 *            {@link #getKey(Uri, int, int)}
	 ...
	 * image before returning it. For non-blocking use, see
	 * {@link #loadImage(int, Uri, int, int)}
	 * 
	 * @param uri
	 * @param type
	 * @param width
	 ...
	 */
	public Drawable getImage(Uri uri, int type, int width, int height)
		...
		try {
			...
			if (MediaFile.isVideoFileType(type)) {
				...
				if (bmp == null)
					bmp = ThumbnailUtils.createVideoThumbnail(uri.getPath(),
							Thumbnails.MICRO_KIND);
				if (bmp == null) {
					throw new ImageCacheException(
							"got null bitmap from request to create video thumb");
				}
				d = new BitmapDrawable(mRes,bmp);
			} else if (type == MediaFile.FILE_TYPE_APK) {
				d = Util.getApkIcon(mContext, uri.getPath());
			} else {
				...
				if (bmp == null) {
					throw new ImageCacheException(
							"got null bitmap from request to scale");
	...
	private class ImageLoadTask implements Runnable, Comparable<ImageLoadTask> {
		...
		public ImageLoadTask(int id, Uri image, int type, int width, int height) {
			this.id = id;
			this.uri = image;
			this.width = width;
			this.height = height;
		...
		public void run() {
			...
			if (DEBUG) {
				Log.d(TAG, "ImageLoadTask.doInBackground(" + id + ", " + uri
						+ ", " + width + ", " + height + ")");
			}

			try {
				final LoadResult result = new LoadResult(id, uri, getImage(uri,
						type, width, height));
				synchronized (jobs) {
					if (jobs.containsKey(id)) {
						// Job still valid.
						jobs.remove(id);
						mHandler.obtainMessage(MSG_IMAGE_LOADED, result)
								.sendToTarget();
					}
				}
	 ...
	 */
	public Drawable loadImage(int id, Uri image, int type, int width, int height)
			throws IOException {
		if (DEBUG) {
			Log.d(TAG, "loadImage(" + id + ", " + image + ", " + width + ", "
					+ height + ")");
		}
		...
		if (res == null) {
			...
			scheduleLoadImage(id, image, type, width, height);
		}
		return res;
	}

	/**
	 * Deprecated to make IDs ints instead of longs. See
	 * {@link #loadImage(int, Uri, int, int)}.
	 * 
	 * @param id
	 * @param image
	 ...
	 */
	// @Deprecated
	// public Drawable loadImage(long id, Uri image, int width, int height)
	// throws IOException {
	 ...
	 *            the maximum height of the resulting image
	 */
	public void scheduleLoadImage(int id, Uri image, int type, int width,
			int height) {
		if (DEBUG) {
			Log.d(TAG, "executing new ImageLoadTask in background...");
		}
		final ImageLoadTask imt = new ImageLoadTask(id, image, type, width,
				height);
	...
	/**
	 * Cancels all the asynchronous image loads. Note: currently does not
	 * function properly.
	 * 
	 */
	...
	private static Bitmap scaleLocalImage(File localFile, int width, int height)
		...
		if (DEBUG) {
			Log.d(TAG, "scaleLocalImage(" + localFile + ", " + width + ", "
					+ height + ")");
		}
		...
		final Bitmap prescale = BitmapFactory.decodeStream(new FileInputStream(
				localFile), null, o2);

		if (prescale == null) {
			Log.e(TAG, localFile + " could not be decoded");
		} else if (DEBUG) {
			Log.d(TAG, "Successfully completed scaling of " + localFile
					+ " to " + width + "x" + height);
		}
	...
	/**
	 * Blocking call to download an image. The image is placed directly into the
	 * disk cache at the given key.
	 * 
	 * @param uri
	...
	protected void downloadImage(String key, Uri uri)
		...
		if (USE_APACHE_NC) {
			...
			// TODO I think this means that the source file must be a jpeg. fix
			// this.
			try {

				putRaw(key, ent.getContent());
				if (DEBUG) {
					Log.d(TAG, "source file of " + uri
							+ " saved to disk cache at location "
	...
	public interface OnImageLoadListener {
		 ...
		 * @param id
		 *            the ID provided by
		 *            {@link ImageCache#loadImage(int, Uri, int, int)} or
